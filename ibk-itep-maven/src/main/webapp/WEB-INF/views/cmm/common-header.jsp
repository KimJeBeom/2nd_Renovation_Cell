<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!doctype html>
<html lang="en">
<head>
	<title>기업은행 IT교육포탈</title>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
	<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">
	<!-- 적용되어야 하는 CSS 파일이 가장 밑으로 가야함 -->
	<!-- VENDOR CSS -->
	<link rel="stylesheet" href="/itep/assets/vendor/semantic/semantic.min.css">
	<link rel="stylesheet" href="/itep/assets/vendor/bootstrap/css/bootstrap.min.css">
	<link rel="stylesheet" href="/itep/assets/vendor/font-awesome/css/font-awesome.min.css">
	<link rel="stylesheet" href="/itep/assets/vendor/linearicons/style.css">
	<link rel="stylesheet" href="/itep/assets/vendor/chartist/css/chartist-custom.css">
	<link rel="stylesheet" href="/itep/assets/vendor/toastr/toastr.min.css">
	<!-- MAIN CSS -->
	<link rel="stylesheet" href="/itep/assets/css/main.css">
	<!-- FOR DEMO PURPOSES ONLY. You should remove this in your project -->
	<link rel="stylesheet" href="/itep/assets/css/demo.css">
	<!-- GOOGLE FONTS -->
	<link href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,600,700" rel="stylesheet">
	<link rel="shortcut icon" type="image/x-icon" href="/itep/assets/itep/img/icon/main_short_cut.png" />
</head>
